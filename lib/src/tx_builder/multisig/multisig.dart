...
class MoneroMultisigTxBuilder
  ...
  factory MoneroMultisigTxBuilder.deserialize(
    List<int> bytes, {
    String? property,
  }) {
  ...
  factory MoneroMultisigTxBuilder.fromStruct(Map<String, dynamic> json) {
    ...
    if (newTx.$1.getTxHash() != tx.getTxHash()) {
  ...
  static MultisigSignatureResponse buildSignature({
    ...
    for (int i = 0; i < sources.length; i++) {
      ...
      final RctKey hL = RCT.hashToP3Bytes(
        signature.signature.mixRing![i][l].dest,
      );
      ...
      CryptoOps.scMul(cachedW[i], mu.$1, sourceKeys.inputSecretKeys[i]);
      CryptoOps.scMulAdd(cachedW[i], mu.$2, z, cachedW[i]);
    }
    ClsagPrunable pronable = signature.rctSigPrunable!.cast<ClsagPrunable>();
    pronable = pronable.copyWith(clsag: clsags);
    final updatedSignature = RCTSignature(
      signature: signature.signature,
      rctSigPrunable: pronable,
    );
  ...
  factory MoneroMultisigTxBuilder({
    ...
    final builder = MoneroMultisigTxBuilder._(
      ...
      multisigInfo: MoneroMultisigSignedInfo.initial(
        signingKeys:
            account.multisigAccount.multisigPrivateKeys
                .map((e) => e.publicKey)
                .toList(),
  ...
  static (MoneroTransaction, List<CLSAGContext>) reConstroctTx({
    required ComputeDestinationKeys destinationKeys,
    required ComputeSourceKeys sourceKeys,
    required List<SpendablePayment<MoneroUnlockedMultisigPayment>> sources,
    required List<MoneroTxDestination> destinations,
    ...
    final KeyV v = List.generate(
      destinationKeys.outs.length,
    ...
    final updatedSignature = sig.copyWith(
      rctSigPrunable: sig.rctSigPrunable!.cast<ClsagPrunable>().copyWith(
        clsag: clsags,
      ),
    );
  ...
  void _init() {
    ...
    for (int j = 0; j < sourceKeys.sourcesLength; j++) {
      final List<MoneroMultisigOutputInfo> otherSignersInfo =
          sources[j].payment.multisigInfos
              .where((e) => signers.contains(e.signer))
              .toList();
      final KeyV alpha = List.generate(
        MoneroMultisigTxBuilder.kAlphaComponents,
        (_) => RCT.zero(),
      );
  ...
  void sign({
    ...
    final KeyM nonces = List.generate(
      sources.length,
