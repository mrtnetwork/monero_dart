...
class MoneroTransactionHelper {
  /// 12 decimals
  static AmountConverter get converter => AmountConverter.kusama;

  static BigInt toPiconero(String amount) {
    return converter.toUnit(amount);
  }

  static String toXMR(BigInt piconero) {
    return converter.toAmount(piconero);
  }

  static List<TxExtra> extraParsing(
    List<int> extera, {
  ...
  static bool hasSameViewTag({
    ...
    final hash = MoneroCrypto.deriveViewTag(
  ...
  static RctKey? isAccountOut({
    ...
    final derivation =
        MoneroCrypto.generateKeyDerivationBigVar(
          pubkey: txPubkey,
          secretKey: viewSecretKey.privateKey.secret,
        ).asImmutableBytes;
    if (hasSameViewTag(
      viewTag: viewTag,
      derivation: derivation,
  ...
  static TxEpemeralKeyResult generateOutputEpemeralKeys({
    ...
    if (additionalSecretKey != null) {
      if (address.isSubaddress) {
        additionalTxPubKey = RCT.scalarmultKey(
          address.pubSpendKey,
          additionalSecretKey.key,
        );
      } else {
    ...
    if (address == changeAddr) {
      derivation = MoneroCrypto.generateKeyDerivation(
        pubkey: txPublicKey,
        secretKey: changeAddressViewSecretKey!,
      );
    } else {
      if (address.isSubaddress && additionalSecretKey != null) {
        derivation = MoneroCrypto.generateKeyDerivationBytes(
          pubkey: address.pubViewKey,
          secretKey: additionalSecretKey.key,
        );
      } else {
        derivation = MoneroCrypto.generateKeyDerivationBytes(
          pubkey: address.pubViewKey,
          secretKey: txSecretKey.key,
        );
      }
    }
    ...
    TxoutTarget? key;
    final viewTag = MoneroCrypto.deriveViewTag(
      derivation: derivation,
      outIndex: outIndex,
    );
  ...
  static MoneroUnlockedOutput? getUnlockOut({
    ...
    final lockedOut = getLockedOutputs(
      tx: tx,
      account: account,
      realIndex: realIndex,
    );
  ...
  static MoneroUnlockedMultisigPayment toMultisigUnlockedOutput({
    ...
    final ownerMultisigInfo = multisigInfos.firstWhere(
      (e) => e.signer == signer,
      orElse: () => multisigAccount.generateMultisigInfo(payment.output).info,
    );
    final otherSigners =
  ...
  static MoneroUnlockedOutput? toUnlockOutput({
    ...
    if (out.accountIndex.isSubaddress) {
      ...
      ephemeralSecretKey = MoneroCrypto.scSecretAdd(
        a: scalarStep1,
        b: subSecretKey,
      );
    } else {
    ...
    if (account.type.isMultisig) {
      ...
      if (out.accountIndex.isSubaddress) {
        ...
        ephemeralPublicKey = MoneroCrypto.addPublicKey(
          ephemeralPublicKey,
          subAddrPk,
        );
      }
    ...
    assert(
      BytesUtils.bytesEqual(out.outputPublicKey, ephemeralPublicKey.key),
      "should be equal.",
    );
  ...
  static List<SpendablePayment<T>> generateFakePaymentOuts<
  ...
  static MoneroTxProof generateOutProof({
    ...
    final prefixHash = _hashProofMessage(
      transaction: transaction,
      message: message,
    );
