...
void _test() {
  ...
  RCT.addKeys2(sp, t, u, RCTConst.h);
  pubs[idx] = CtKey(dest: sI, mask: sp);
  // Set commitment offset
  final RctKey cout = RCT.zero();
  t2 = RCT.skGen_();
  RCT.addKeys2(cout, t2, u, RCTConst.h);

  // Prepare generation inputs
  final CtKey insk = CtKey(dest: p, mask: t);
  final Clsag prove = CLSAGUtins.prove(message, pubs, insk, t2, cout, idx);
  final bool vr = CLSAGUtins.verify(message, prove, pubs, cout);
  test("correct sig", () {
    expect(vr, true);
  ...
  test("bad message", () {
    final prove = CLSAGUtins.prove(RCT.zero(), pubs, insk, t2, cout, idx);
    final vr = CLSAGUtins.verify(message, prove, pubs, cout);
    expect(vr, false);
  });

  test("bad index", () {
    final prove =
        CLSAGUtins.prove(message, pubs, insk, t2, cout, (idx + 1) % N);
    final vr = CLSAGUtins.verify(message, prove, pubs, cout);
    expect(vr, false);
  });
  ...
  test("bad C", () {
    pubs[idx] =
        CtKey(dest: RCT.scalarmultBase_(RCT.skGen_()), mask: pubs[idx].mask);
    final prove =
        CLSAGUtins.prove(message, pubs, insk, t2, cout, (idx + 1) % N);
    final vr = CLSAGUtins.verify(message, prove, pubs, cout);
    expect(vr, false);
